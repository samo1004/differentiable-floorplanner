...
def run_single(case_path: str, strategy: str, seed: int,
               iterations: int, device: torch.device, quiet: bool = True):
    ...
    overlap_ratio = overlap / chip_area * 100  # percentage
...
def select_top_k(results: list, k: int = 5) -> list:
    ...
    # Prefer legal (overlap < 1) solutions; break ties by HPWL
...
def average_metrics(results: list) -> dict:
    ...
    keys = ['hpwl', 'overlap', 'overlap_ratio', 'boundary_total', 'ar_violations', 'elapsed']
    avg = {}
    for k in keys:
...
def main():
    ...
    parser.add_argument('--cases', nargs='+', type=int, default=[1, 2, 3, 4, 5, 6],
                        help='Case numbers to run (default: 1 2 3 4 5 6)')
    parser.add_argument('--strategies', nargs='+', default=['escalating', 'fixed'],
    ...
    print(f"╔══════════════════════════════════════════════════════════════╗")
    print(f"║          Differentiable Global Placer — Benchmark          ║")
    print(f"╠══════════════════════════════════════════════════════════════╣")
    ...
    all_summaries = []  # for final table
    best_case06 = None  # track best case06 run for GIF
    ...
    for case_num in args.cases:
        ...
        for strategy in args.strategies:
            ...
            for si, seed in enumerate(seed_list):
                ...
                sys.stdout.flush()
                r = run_single(case_path, strategy, seed, args.iterations, device)
                results.append(r)
            print()
            ...
            # Track best case06 fixed for GIF
            ...
            print(f"  Top-{args.top_k} avg HPWL: {avg['hpwl']:,.0f}  |  "
    ...
    print("=" * 80)
    print()
    print("| Case | Strategy | Avg HPWL | Best HPWL | Overlap Ratio | "
          "Boundary Viol | AR Viols | Avg Time (s) | Legal |")
    print("|------|----------|----------|-----------|---------------|"
          "--------------|----------|--------------|-------|")
    for s in all_summaries:
        legal_str = f"{s['legal_count']}/{s['total_count']}"
        print(f"| {s['case']} | {s['strategy']:10s} "
              f"| {s['hpwl']:>10,.0f} "
              f"| {s['best_hpwl']:>10,.0f} "
              f"| {s['overlap_ratio']:>12.4f}% "
              f"| {s['boundary_total']:>12.1f} "
              f"| {s['ar_violations']:>8.1f} "
              f"| {s['elapsed']:>12.2f} "
              f"| {legal_str:>5s} |")
    print()
    ...
    if args.json:
        ...
        for s in all_summaries:
            json_data.append({
                ...
                'strategy': s['strategy'],
                'avg_hpwl': s['hpwl'],
                'best_hpwl': s['best_hpwl'],
                'avg_overlap_ratio': s['overlap_ratio'],
                'avg_boundary': s['boundary_total'],
                'avg_ar_violations': s['ar_violations'],
                'avg_elapsed': s['elapsed'],
