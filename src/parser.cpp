...
static int add_module(Problem& P, Module m, const std::string& file, const std::string& section, int idx1) {
    if (P.name2id.count(m.name)) {
        throw std::runtime_error("ParseError[file=" + file + "][" + section + " #" + std::to_string(idx1) +
                                 "]: duplicate module name '" + m.name + "'");
    }
    int id = (int)P.modules.size();
    P.modules.push_back(std::move(m));
    P.name2id[P.modules[id].name] = id;
    return id;
}

Problem parse_problem(const std::string& path) {
    std::ifstream fin(path);
    if (!fin) throw std::runtime_error("ParseError: cannot open file: " + path);

    Problem P;
    std::string tag;

    // CHIP w h
    fin >> tag; expect_tag(tag, "CHIP", path);
    fin >> P.chip_w >> P.chip_h;

    // SOFTMODULE n
    fin >> tag; expect_tag(tag, "SOFTMODULE", path);
    int n_soft = 0; fin >> n_soft;
    for (int i = 0; i < n_soft; ++i) {
        Module m; m.type = ModuleType::Soft;
    ...
    // FIXEDMODULE m
    fin >> tag; expect_tag(tag, "FIXEDMODULE", path);
    int n_fixed = 0; fin >> n_fixed;
    for (int i = 0; i < n_fixed; ++i) {
        Module m; m.type = ModuleType::Fixed;
    ...
    // CONNECTION k
    fin >> tag; expect_tag(tag, "CONNECTION", path);
    int k = 0; fin >> k;
